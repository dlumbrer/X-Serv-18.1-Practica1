...
class acortadorApp(webapp.webApp):
	...
	def process(self, (metodo, recurso, cuerpo)):
		...
		formulario = "<form method='post'>Introduce url:<input type='text' name='url' value='' /><button type='submit'>Acortar!</button></form>"
		...
		if not contenido.startswith("http"):
			...
			contenido = contenido.replace("%3A", ":").replace("%2F","/").replace("%3F", "?").replace("%3D", "=")
